...
class ImageCaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_path = f'C:\internship project\Image Captioning Application\image\{pattern_removed}'
...
model_path = 'C:\internship project\Image Captioning Application\ trained_model.pth'
model.load_state_dict(torch.load(model_path))

# Function to generate captions for a new image using the trained model
def generate_caption(image_path, model, transform, vocab, max_length=50):
    try:
        image = Image.open(image_path).convert('RGB')
    ...
    with torch.no_grad():
        ...
        for _ in range(max_length):
            ...
            word_idx = predicted[0][-1].item()
            if word_idx == vocab['<end>']:
                break
            word = [key for key, value in vocab.items() if value == word_idx]
...
# Example usage
image_path = r'C:\internship project\Image Captioning Application\Oceans35 (1).jpg'
